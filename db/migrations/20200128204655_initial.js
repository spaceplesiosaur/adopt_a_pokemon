//this databases themselves are generated CREATE DATABASE, which is a command in the PostGreSQL library.  This file, however, is not.  It is generated when we tell knex to make an initial migration with
...
//exports up is a knex function that tells your app what changes to make when this migration runs.  In this case, we are making two tables - natures and pokemon.
  return knex.schema
  ...
  //createTable is a .schema method that takes a database name from PostGres (remember, we created it with CREATE DATABASE, and a callback function that then takes that table in the first argument and turns it into a knex stateful table object with all of the schema builder functions available to it.)
    ...
    .createTable('pokemon', function (table) {
      ...
      //this is how we connect tables.  Here, we specifying that our newly created nature_id column's value is not only an integer, but it is a foreign key.  It is an integer that represents a value on another table.
        ...
        //this is where we specify what the foreign key represents.  'natures' is a table we already created, and .id is the id column.
      table.timestamps(true, true);
    })
};
